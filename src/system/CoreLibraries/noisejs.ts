...
// adjusted to support ES modules, use a class, and support typescript types
...
export class NoiseJS {
	...
	#F2: number = 0.5 * (Math.sqrt(3) - 1);
	#G2: number = (3 - Math.sqrt(3)) / 6;
	...
	perlin3(x: number, y: number, z: number) {
		...
		let n111 = this.#gradP[
			X + 1 + this.#perm[Y + 1 + this.#perm[Z + 1]]
		].dot3(x - 1, y - 1, z - 1);
