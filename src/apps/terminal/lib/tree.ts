...
async function treeWalk(
	...
	const resp = await parent.env.fs.listDirectory(directory);
	if (!resp.ok) {
		return;
	}
	...
	for (const i in contents) {
		...
		if (/*(*/ file[0] !== "." /*) || (obj.showHidden)*/) {
			const parts = Number(i) == contents.length - 1 ? ["└── ", "    "] : ["├── ", "│   "];
			...
			const resp = await parent.env.fs.stat(asDir);
			if (!resp.ok) {
				continue;
			}
			...
			if (isDir) {
				...
				counts.dirs++;
				result += await treeWalk(parent, asDir, prefix + parts[1], maxDepth, depth + 1, counts);
			} else {
				/*if (!obj.dirOnly) {*/
				result += prefix + parts[0] + dispFile + `\n`;
				/*};*/
