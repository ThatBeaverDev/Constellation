/**
 * Code is from the Constellation Repository!
...
const runesToStr = (runes: Rune[]) => runes.map((r) => String.fromCodePoint(r)).join("");
...
function parseTerms(fuzzy: boolean, caseMode: Casing, normalize: boolean, str: string): TermSet[] {
 ...
 const buildPatternForBasicMatch = (query: string, casing: Casing, normalize: boolean) => {
...
type SyncOptsToUse<U> = BaseOptsToUse<U> & Partial<Pick<SyncOptions<U>, "match">>;
...
type AsyncOptsToUse<U> = BaseOptsToUse<U> & Partial<Pick<AsyncOptions<U>, "match">>;
