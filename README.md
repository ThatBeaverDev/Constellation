...
If you are trying to run a version from before here - note you need another repository. Running this version is _not_
supported at this time, and you're on your own.
...
Otherwise, I trust you know how to use checkout to get to another commit - it is recommended to look for groups and go
for the last one, since they may be out of order.
...
Since Constellation uses TypeScript for large parts of the code, it must be compiled. Constellation can be compiled with
this command:
...
However, this recompiled _everything_. To just recompile Apps, you can just run this command, which should compile
faster:
