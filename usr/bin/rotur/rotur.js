...
class RoturExtension {
	  ...
	  fetch("https://raw.githubusercontent.com/RoturTW/main/main/rotur.json")
		.then((response) => {
		  if (response.ok) {
			...
			system.error('Network response was not ok',"Rotur");
		  }
		})
		.then((data) => {
	...
	sendMessage(PAYLOAD, SOURCE, TARGET, USER) {
	  if (!this.is_connected) {
	...
	getUserDesignation(DESIGNATION) {
	  if (!this.is_connected) {
		return "Not Connected";
	  }
	  return JSON.stringify(
		this.client.users.filter((user) =>
		  user.startsWith(DESIGNATION + "-"),
		),
	  );
	}
...
async function frame() {
	if (system.rotur.getToken() == "") {
		return
	}
	let packets = system.rotur.RAWgetAllPackets()
	system.rotur.RAWdeleteAllPackets()
